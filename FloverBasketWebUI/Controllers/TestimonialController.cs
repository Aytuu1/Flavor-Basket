...
namespace FloverBasketWebUI.Controllers
{
  public class TestimonialController : Controller
  {
    ...
    public async Task<IActionResult> CreateTestimonial(CreateTestimonialDto createTestimonial)
    {
      ...
      var responseMessage = await client.PostAsync("https://localhost:7230/api/Testimonial", stringContent);
      if (responseMessage.IsSuccessStatusCode)
      {
    ...
    {
      var client = new HttpClient(handler);
      var responseMessage = await client.DeleteAsync($"https://localhost:7230/api/Testimonial/{id}");
      if (responseMessage.IsSuccessStatusCode)
      {
        return RedirectToAction("Index");
      }
    ...
    {
      var client = new HttpClient(handler);
      var responseMessage = await client.GetAsync($"https://localhost:7230/api/Testimonial/{id}");
      if (responseMessage.IsSuccessStatusCode)
      {
        var jsonData = await responseMessage.Content.ReadAsStringAsync();
    ...
    public async Task<IActionResult> UpdateTestimonial(UpdateTestimonialDto updateTestimonial)
    {
      ...
      StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
      var responseMessage = await client.PutAsync("https://localhost:7230/api/Testimonial/", stringContent);
      if (responseMessage.IsSuccessStatusCode)
      {
        return RedirectToAction("Index");
      }
